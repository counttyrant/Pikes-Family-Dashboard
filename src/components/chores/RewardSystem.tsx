...
export default function RewardSystem({ selectedMemberId, locked = true, onActivity }: RewardSystemProps) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* inline add form -- only when unlocked */}
      {showForm && !locked && (
        ...
        {rewards.length === 0 && !showForm && (
          <div className="text-center text-slate-500 py-12">
            ...
            <p className="text-sm mt-1">
              {locked ? 'Tap the lock icon in the header to manage rewards' : 'Add rewards to motivate the family!'}
            </p>
          </div>
        )}
      </div>
