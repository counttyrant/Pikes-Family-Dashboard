...
export function LoginScreen() {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950">
      ...
      {sessionExpired && user ? (
        /* ── Reconnect card (session expired but profile known) ── */
        <div className="relative z-10 mx-4 w-full max-w-sm rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl text-center">
          <span className="mb-4 inline-block text-4xl" role="img" aria-label="house">🏠</span>
          ...
          <div className="my-5 flex flex-col items-center gap-2">
            {user.picture ? (
              <img
                src={user.picture}
                alt={user.name}
                className="h-16 w-16 rounded-full ring-2 ring-indigo-400/50"
              />
            ) : (
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-indigo-600 text-2xl font-bold text-white ring-2 ring-indigo-400/50">
          ...
          <p className="mb-5 text-sm text-slate-400">Your session expired. Tap below to reconnect.</p>

          <button
            type="button"
            onClick={handleSignIn}
            disabled={showSpinner}
            className="group relative flex w-full items-center justify-center gap-3 rounded-xl bg-indigo-600 px-6 py-3.5 text-sm font-semibold text-white shadow-lg transition-all duration-200 hover:-translate-y-0.5 hover:bg-indigo-500 hover:shadow-xl disabled:pointer-events-none disabled:opacity-60"
          >
            {showSpinner ? (
